...
1. センサーデータ取得及びコンテナ化機能でスマートフォンのセンサ情報をコンテナで取り込みます。
2. メッセージング機能を介してコンテナ処理機能にコンテナデータを送ります。
3. コンテナ処理機能はスキーマリポジトリ機能と連携して、コンテナから JSON に変換します。
...
1. スキーマリポジトリ機能でのスキーマ確認  
   テストラボで準備しているスキーマリポジトリ機能には以下の機能があります。
...
1. 読み込み結果のプレビュー
   ...
   サンプルコンテナに対応するスキーマ情報は、リポジトリ内で事前に定義されているため、対応するサンプルコンテナを読み込むことで、スキーマリポジトリ機能の動作確認ができます。
...
スマートフォンからジャイロ(傾き)と加速度のセンサデータを送るための設定をします。

![ngrok](environment/overview_target7.drawio.png)

_図 G-1:システム構成における設定箇所_

センサーデータ取得及びコンテナ化機能はブラウザでアクセスする機能です。
スマートフォンのブラウザから https でアクセスすることでセンサのデータを送信できるようになります。
...
センサーデータ取得及びコンテナ化機能について、センサデータを**連続的に**送信するための設定をします。

![ngrok](environment/overview_target8.drawio.png)

_図 G-3:システム構成における設定箇所_

ここでは、センサデータへブラウザからアクセスできるものとして手順を進めます。
...
ダイアログで許可をすると、画面上の加速度や傾きがセンサから取得されるデータになります。
