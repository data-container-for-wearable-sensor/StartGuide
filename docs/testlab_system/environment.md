...
1. スマートフォンセンサ機能：[http://localhost:1188/](http://localhost:1188/)
...
トピックの作成のために、スマートフォンセンサ機能からメッセージング機能にデータを送ります。
...
1. 読み込み結果のプレビュー
   ...
   サンプルコンテナに対応するスキーマ情報は、リポジトリ内で事前に定義されているため、対応するサンプルコンテナを読み込むことで、スキーマリポジトリの動作確認ができます。
