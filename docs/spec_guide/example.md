...
また、ここでは拡張パート(Extended Part)を要さないコンテナを作ります。
...
1. Container Length  
   ...
   ここでは、`(0x02 + 0x02 + 0x01 + 0x01 + 0x10) + 0x38 = 0x4E` かつ、 Container Length は 2byte 幅なので`0x004E`となります。
...
以上で、ヘッダのコモンパートとペイロードを用いてコンテナを作成できる。

## コンテナデータの利用方法

構成図の右側に注目し、コンテナを受信して必要な情報を取得する手順について説明する。

![](./overview_right.drawio.png)

_図 3-4: 構成図上での注目ポイント_
...
その後、直前の例で示したコンテナデータ利用の具体例を説明します。
...
下図 3-7:はスキーマ情報を用いて、ペイロードから情報を取り出す手順を示しています。

![スキーマの取得](container_parse.drawio.png)

_図 3-7: スキーマ情報を利用したペイロードの利用_

スキーマ情報によって、ペイロードの中から、スキーマ情報上の名前とセンサデータの組が取得できます。
...
最期の工程で、切り分けたバイト列を `type` に従って値にしたものが表 3-5:になります。
