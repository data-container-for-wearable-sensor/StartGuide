...
従来、センサ装置やデバイスからデータを取得する際、
各ベンダーごとの独自のフォーマットでデータが届くことが通常であり、そこに標準化されたフォーマットは存在しませんでした。
...
そこで、ペイロードから情報を取り出すためのメタデータ(構造や型)をスキーマ情報と呼び、コンテナの外部に定義します。

コンテナとスキーマ情報を組み合わせることで、決まった手続きを行うことで、コンテナのペイロードから情報を取り出すことができます。
...
コモンパーとに含まれる[コンテナタイプ](#container-type)のうち、Extended Attributes が `YES` の場合は、
コモンパートの後に拡張パートが続きます。  
...
ペイロードには通常何らかの情報が抽出できるバイト列が格納されています。
...
センサ装置等、外部から受取ったコンテナを処理に用いるためには、以下の手順を踏む必要があります。

- コンテナから情報を取り出す。
- スキーマリポジトリとの連携
- スキーマ情報の適用

具体的なデータを含めた例は、[事例ページ](./spec_guide/example)を参照してください。
