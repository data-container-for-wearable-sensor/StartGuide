...
図 1: 中 (1)で可視化画面を開きます。
センサーデータを受信して可視化するための画面をブラウザで表示します。
...
このページを開いたままスマートフォンからセンサーデータを送信することで、データの可視化を確認します。
...
_図 4: スマートフォンのモーション許可ダイアログ _

`"動作と方向"へのアクセス` を許可することで、
ジャイロセンサーの情報が画面に反映されるようになり、画面上の加速度/傾きの値が目まぐるしく変化するようになります。
...
送信されたデータは[センサーデータの可視化画面の起動と設定](#センサーデータの可視化画面の起動と設定)の項で開いた画面で確認できます。

![](testlab_preview.png)

_図 6: 可視化されたセンサーデータのグラフ_

上のグラフは加速度センサーの値、下のグラフは傾きセンサーの値を示しています。
...
**最も基本的な形のコンテナは決まったヘッダをペイロードに付けること**で、コンテナデータになることである。

### コンテナヘッダの扱い方

コンテナ処理機能では、外部から受け取ったコンテナデータを処理するために、以下のようにコンテナデータからヘッダの情報を取り出す。
以下の抜粋はコンテナデータを受取って、ヘッダの各フィールドとペイロードを取り出す処理である。
Python3 による実装です。
...
この抜粋で示すことは、 [コンテナフォーマット 使用例 ヘッダとペイロードの分割](spec_guide/example#ヘッダとペイロードの分割)で、示したように
コンテナヘッダの仕様に基づいて、決まったバイト位置から決まった幅のバイト列を取り出す方法で、
コンテナヘッダのフィールドやコンテナのペイロードを取り出す実装の一つを示している。

ここでは手続きによってコンテナデータをヘッダのフィールド群やペイロードとして取り出したが、
例えば C 言語であれば構造体へのキャストでも同様の処理が実現可能である。

:::caution
ここで作られているコンテナは[Container Format で示された仕様](spec_guide)と差異がある。

コンテナヘッダ内の Data ID の長さは Data ID Length で示す仕様である。
しかし、Data ID の長さは Data Index の値に対応した値になるよう取り扱っている。
:::

### スキーマ情報の確認方法

スキーマリポジトリの設定画面からスキーマ情報を確認します。

[Download(コンテナ例)](mobile_acce.cntr)ファイルがスマートフォンセンサ機能から出力されるコンテナと同じ構造です。

ダウンロードしたコンテナ例のファイルをスキーマリポジトリの画面で読み込むことで、
コンテナデータに対応するスキーマ情報をサーバから呼び出し、読み込んだコンテナに適用して表示します。

画面左が、コンテナデータのバイナリを Hex 記法(16 進数)で表示した領域です。
画面右が、コンテナデータに対応するスキーマ情報の内容表示およびスキーマの編集を行う領域です。

![](iot-repository-example.png)
_図 7: スキーマ情報の編集画面_

スキーマ情報はこの画面から更新する事もできます。
...
フィールドの一番右のボタンでフィールドを削除することもできます。

### スキーマ情報の例 と 利用のされかた

スマートフォンセンサ機能から出力されるコンテナデータは以下の図表の構造のペイロードが含まれています。

![](./payload_example.png)

_図 8: スマートフォンセンサ機能のペイロードの構造_

_表 1: スマートフォンセンサ機能のペイロードのフィールドの情報_
...
このスキーマ情報は、テストラボシステムのスキーマリポジトリが作るファイルフォーマットです。
...
スキーマリポジトリの画面ではフィールド定義に従って、ペイロードの分割および型を設定してプレビューしている、以下がその例である。

![フィールド例](schema_field.png)

_図 9: スキーマリポジトリ画面での dt の例_

図 9: において、`0x00000183ac9b6882` というバイト列を、フィールドの `pos`(オフセット)と `length`(データ幅)で取り出して、`type`で 8 バイトの符号なし整数として解釈した結果が `1665048209538`(符号なし整数) である。

このように、スキーマ情報のフィールド定義を基にコンテナのペイロードから名前と型のあるデータを取り出して使うことができます。

以上で、テストラボシステムにおけるスキーマ情報の実装例及びスキーマ情報の利用方法の例、スキーマリポジトリ上での表示の説明を示しました。
